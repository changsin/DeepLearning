...
#                         metrics.to_rect(y_gt[i]*IMAGE_SIZE))
    iou = metrics.intersection_over_union(torch.from_numpy(y_preds_avg[i]),
     torch.from_numpy(y_gt[i])).item()
...
def plot_stats_by_type(stats_data, x_bins, column, title, K=5, merge=False):
  fig = plt.figure(figsize=(20,40))
